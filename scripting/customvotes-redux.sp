...
public OnClientDisconnect(iTarget)
{
	...
	for(new iVote = 0; iVote < MAX_VOTE_TYPES; iVote++)
	{
		switch(g_iVoteType[iVote])
		{
			...
			{
				for(new iVoter = 1; iVoter <= MaxClients; iVoter++) if(IsClientInGame(iVoter))
					CheckVotesForTarget(iVoter, iVote);
			}
			case VoteType_Map:
			{
				for(new iMap = 0; iMap < MAX_VOTE_MAPS; iMap++)
...
public Menu_ChooseVote(iVoter)
{
	...
	for(new iVote = 0; iVote < g_iVoteCount; iVote++)
	{
		...
		// Max votes
		else if(g_iVoteRemaining[iVoter][iVote] <= 0 && g_iVoteMaxCalls[iVote] > 0)
			iFlags = ITEMDRAW_DISABLED;
		...
		// Cooldown
		else if(iTime - g_iVoteLast[iVoter][iVote] < g_iVoteCooldown[iVote])
			iFlags = ITEMDRAW_DISABLED;
...
public MenuHandler_Vote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		switch(g_iVoteType[iVote])
		{
			...
			case VoteType_Map: Menu_MapVote(iVote, iVoter);
			case VoteType_List: Menu_ListVote(iVote, iVoter);
			case VoteType_Simple: CastSimpleVote(iVoter, iVote);
		}
	}
}
...
public Menu_PlayersVote(iVote, iVoter)
{
	...
	if(g_iVoteRemaining[iVoter][iVote] <= 0 && g_iVoteMaxCalls[iVote] > 0)
	{
	...
	new iTime = GetTime();
	if(iTime - g_iVoteLast[iVoter][iVote] < g_iVoteCooldown[iVote])
	{
...
public CastSimpleVote(iVoter, iVote)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
...
stock FormatTargetString(iVote, bool:bQuotes = false, iTarget, String:strBuffer[], iBufferSize)
{
	...
	ReplaceString(strBuffer, iBufferSize, "{VOTER_STEAMID}", strTargetSteamId, false);
